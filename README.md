...
Finyu memakai **OAuth Google** sebagai login sosial, menggantikan "Sign in with
Viktor" (`viktor.space.json` → `auth.providers: ["email_password"]`, provider
`viktor` juga disaring di `convex/auth.ts`). Kodenya ada di
`convex/googleAuth.ts` + `src/components/GoogleSignInSection.tsx`.

**Cara mengaktifkan**

1. Google Cloud Console → *APIs & Services* → *Credentials* → *Create
   credentials* → *OAuth client ID* → tipe **Web application**.
2. Isi **Authorized redirect URIs** (daftarkan keduanya):
   - preview  : `https://oceanic-wolf-528.convex.site/api/auth/callback/google`
   - produksi : `https://frugal-fly-380.convex.site/api/auth/callback/google`
3. Set kredensialnya di deployment Convex:
   ...
   # untuk produksi: tambahkan --prod
   ```

Begitu dua env var itu ada, tombol **"Masuk dengan Google"** otomatis muncul di
halaman masuk/daftar (query `auth.googleSignInEnabled` yang menentukan). Kalau
belum di-set, tombolnya disembunyikan dan login email + kata sandi tetap jalan.
Akun Google dengan email yang sama dengan akun email/password yang sudah ada
akan digabung (`allowDangerousEmailAccountLinking`).
...
## Aplikasi HP (PWA, Android, iOS)

Finyu sudah dibundel sebagai **PWA (Progressive Web App)** — bisa dipasang ke
layar utama dan jalan layar penuh tanpa address bar, tanpa perlu app store.

### Berkas yang menyusun lapisan PWA

| Berkas | Fungsi |
| --- | --- |
| `public/manifest.webmanifest` | Nama, ikon, warna tema, `display: standalone`, `start_url: /dashboard`, shortcut ke Pengeluaran/Tabungan/Laporan |
| `public/sw.js` | Service worker: app shell offline, cache-first untuk aset statis, **tidak pernah** meng-cache domain lain (Convex/Google/gateway WA) |
| `src/pwa/registerServiceWorker.ts` | Registrasi SW (hanya di build produksi/preview), toast "versi baru tersedia", helper `isStandalone()` / `isIos()` |
| `src/components/finyu/InstallAppPrompt.tsx` | Banner "Pasang Finyu" (`beforeinstallprompt` di Android, instruksi Bagikan → Tambahkan ke Layar Utama di iOS) |
| `public/icon-*.png`, `public/apple-touch-icon.png` | Ikon 192/512 + maskable + ikon iOS |
| `index.html` | `<link rel="manifest">`, `theme-color`, meta `apple-mobile-web-app-*`, `viewport-fit=cover` |

Syarat pemasangan: situs harus HTTPS (URL Space sudah HTTPS), manifest valid,
dan service worker aktif — ketiganya diverifikasi otomatis oleh
`bun run test scripts/finyu-pwa.ts`.

### Memasang di HP

- **Android (Chrome/Edge):** buka URL app → banner "Pasang Finyu" muncul, atau
  menu ⋮ → *Install app*.
- **iPhone/iPad (Safari):** tombol Bagikan → *Tambahkan ke Layar Utama*.
- **Desktop (Chrome/Edge):** ikon install di address bar.

Catatan: data keuangan tetap butuh koneksi karena tersimpan di Convex. Saat
offline, cangkang aplikasi tetap terbuka dan menampilkan pesan yang jelas.

### Kalau butuh file APK/AAB (Google Play) — Capacitor

Cara paling murah mengubah app ini jadi aplikasi native tanpa menulis ulang UI:

```bash
bun add -d @capacitor/cli
bun add @capacitor/core @capacitor/android @capacitor/app @capacitor/browser
bunx cap init Finyu com.finyu.app --web-dir=dist
bun run build                 # menghasilkan dist/
bunx cap add android
bunx cap sync
bunx cap open android         # build APK/AAB di Android Studio
```

Dua pilihan cara memuat aplikasi di dalam shell native:

1. **Bundel `dist/` ke dalam APK** (default). Update UI = rilis versi baru.
2. **Arahkan ke URL live** — di `capacitor.config.ts`:
   `server: { url: "https://<url-space>", cleartext: false }`.
   Update UI langsung terasa tanpa rilis ulang; APK-nya hanya pembungkus.

Hal yang perlu diperhatikan di build native:

- **Login Google di webview:** Google menolak OAuth di dalam webview biasa.
  Pakai `@capacitor/browser` (Custom Tabs / SFSafariViewController) untuk
  membuka halaman login, dan tangani kembalinya lewat deep link
  (`appUrlOpen`). Login email + kata sandi jalan tanpa penyesuaian.
- **Play Console** butuh akun developer sekali bayar $25 (Apple $99/tahun).
- Alternatif tanpa Android Studio: **PWABuilder** (`pwabuilder.com`) bisa
  membungkus PWA ini jadi AAB Trusted Web Activity langsung dari URL-nya.

### Kalau butuh rasa native penuh — React Native / Expo

Backend Convex bisa dipakai apa adanya (`convex` mendukung React Native), yang
ditulis ulang hanya lapisan UI-nya. Ini pilihan terbaik kalau butuh push
notification andal di iOS, widget, atau akses kontak/kamera; ongkosnya jauh
lebih besar daripada dua opsi di atas.
